// Package dynamitejobs implements provider.Provider for the Dynamite Jobs
// remote job board via its WordPress RSS feed.
//
// Dynamite Jobs is a search-only aggregator: postings link to the employer's
// own apply page, so Apply always returns "skipped".
package dynamitejobs
...
const feedURL = "https://www.dynamitejobs.com/feed/"
...
// Client implements provider.Provider for Dynamite Jobs.
type Client struct {
	http *http.Client
...
// New creates a Dynamite Jobs client.
func New() *Client {
	return &Client{http: &http.Client{Timeout: 30 * time.Second}, feedURL: feedURL}
}

func (c *Client) Name() string { return "dynamitejobs" }
...
func (c *Client) Search(ctx context.Context, criteria provider.SearchCriteria) ([]provider.Job, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("dynamitejobs: HTTP %d", resp.StatusCode)
	}

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("dynamitejobs: read: %w", err)
	}

	var feed rssFeed
	if err := xml.Unmarshal(body, &feed); err != nil {
		return nil, fmt.Errorf("dynamitejobs: xml parse: %w", err)
	}
	...
	for _, item := range feed.Items {
		...
		// Dynamite Jobs titles look like "Job Title at Company".
		company := "Dynamite Jobs"
		jobTitle := title
...
// Apply marks as skipped — Dynamite Jobs postings link to the employer's own
// apply page.
