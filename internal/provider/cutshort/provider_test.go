...
func TestName(t *testing.T) {
	c := New()
	if c.Name() != "cutshort" {
		t.Errorf("Name() = %q; want \"cutshort\"", c.Name())
	}
}
	...
	}{
		{
			...
			scrape: func(_ context.Context, _, _ string, _ []string, _ bool) ([]scraper.BoardJob, error) {
				return []scraper.BoardJob{
					{Title: "Backend Engineer", Company: "Acme", Location: "Remote", ApplyURL: "https://example.com/jobs/1", Remote: true},
					{Title: "Engineer II", Company: "Beta", Location: "Remote", ApplyURL: "https://example.com/jobs/2", Remote: true},
				}, nil
			},
			wantJobs: 2,
		},
		{
			...
			scrape: func(_ context.Context, _, _ string, _ []string, _ bool) ([]scraper.BoardJob, error) {
				return []scraper.BoardJob{
					...
					{Title: "Sales Rep", Company: "B", Location: "Remote", ApplyURL: "https://example.com/jobs/2"},
				}, nil
			},
			wantJobs: 1,
		},
