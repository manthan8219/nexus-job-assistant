...
// handleLLMPickerKey navigates the model list. Up at top / down at bottom leave the field.
	...
	case "install-ollama", "install-lmstudio":
		...
		return func() tea.Msg {
			if err := localllm.OpenURL(download); err != nil {
				return localLLMPullDoneMsg{Err: err} // reuse to show error on form
			}
...
func (m *FormModel) refreshLLMOptions(installed []string) {
	...
	// Prefer currently selected model, else Best.
...
func (m FormModel) renderLocalLLMPicker(active bool) string {
	...
	for i, r := range m.llmOptions {
		...
		if i == m.llmCursor {
			mark = "▶ "
			line = lipgloss.NewStyle().Foreground(lipgloss.Color(colorPurple)).Bold(true).Render(label) + "  " + mutedStyle.Render(meta)
		} else if r.Installed {
...
func (m FormModel) renderLLMSetupMenu(hw string) string {
	...
	for i, opt := range localllm.SetupOptions() {
		...
		if i == m.llmSetupCursor {
			mark = "▶ "
			line = lipgloss.NewStyle().Foreground(lipgloss.Color(colorPurple)).Bold(true).Render(label) + "  " + mutedStyle.Render(hint)
		} else {
			line = mutedStyle.Render(label + "  " + hint)
		}
		b.WriteString("\n    " + mark + line)
	}
	b.WriteString("\n    " + mutedStyle.Render("↑↓ move · enter run · ↑ on first / shift+tab / esc leaves · tab next"))
	return b.String()
}
...
func (m FormModel) renderScraperSetupMenu() string {
	...
	venvReady := scraper.Installed()
	if !venvReady {
		// Step 1: venv not set up yet — show install-venv action
	...
	for i, b2 := range scraper.Catalog {
		isInstalled := installedSet[b2.ID]
		mark := "  "
		var line string
		meta := b2.Notes
		if isInstalled {
		...
		if i == m.scraperBackendCursor {
			mark = "▶ "
			label := lipgloss.NewStyle().Foreground(lipgloss.Color(colorPurple)).Bold(true).Render(b2.Name)
			line = label + "  " + mutedStyle.Render(meta)
		} else if isInstalled {
			line = primaryStyle.Render(b2.Name) + "  " + mutedStyle.Render(meta)
		} else {
			line = mutedStyle.Render(b2.Name + "  " + meta)
		}
		b.WriteString("\n    " + mark + line)
	}
	b.WriteString("\n    " + mutedStyle.Render("↑↓ move · enter install · tab next"))
...
// renderAISectionHint explains the two AI setup paths when assist is on.
...
// renderAIProvider renders Local LLM vs API Keys.
